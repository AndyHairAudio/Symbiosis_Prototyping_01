...
public class TextSequencerForest : MonoBehaviour {
	...
	void Update () {
		if (tooltipTriggerComp.playerNearForestMonolith) {
			gameObject.GetComponent<Image> ().enabled = true;
			if (!sequenceRunning) {
				if (Input.GetButtonDown ("Fire1")) {
					sequenceRunning = true;
					StartCoroutine ("ForestSequence");
				}
	...
	IEnumerator ForestSequence (){
			...
			textCompChildren [4].enabled = true;

			while (!Input.GetButtonDown ("Fire1")) {
					yield return null;
			}

			textCompChildren [4].enabled = false;
